...
const IMAGE_STUB_URL = path.resolve(__dirname, '../imageStub.png');
const IMAGE_DATA_BUFFER = fs.readFileSync(IMAGE_STUB_URL);
const VIDEO_URL_RE = /\.mp4/i;
const VIDEO_STUB_URL = path.resolve(__dirname, '../videoStub.mp4');
const VIDEO_DATA_BUFFER = fs.readFileSync(VIDEO_STUB_URL);
const HEADERS_STUB = {};
...
module.exports = async function(page, scenario) {
  const intercept = async (request, targetUrl) => {
    ...
    } else if (VIDEO_URL_RE.test(request.url())) {
      await request.respond({
        body: VIDEO_DATA_BUFFER,
        headers: HEADERS_STUB,
        status: 200,
      });
    } else {
